...
class ClassyModel(object):
    ...
    def split_n_fit(self, X, y, test_size, random_state):
...
def simulate(model, label, target_p, test_sizes, n, random_state):
    """
    We run this n times.
    ...
    scores_i = np.zeros((n, len(test_sizes)))
    for i in range(n):
        X, y, vocab = load_data(label=label, target_p=target_p, min_df=0.005)
        scores = []
        for test_size in test_sizes:
            CM.split_n_fit(X, y, test_size, random_state)
            score = CM.score()
